...
kDataPath <- file.path("~/src/boston/research/product/deep_dive/Rules_lapse_rate_correlations_20200917","data")
...
select distinct
  ...
  a.activity_end,
  a.eight_weeks,
  case when r.user_id is not null then r.created else null end as rules_user
from activity a
left join rules_users r on r.user_id = a.user_id
",'redshift-eu')

mau_rules <- as.data.table(mau_rules)

save(mau_rules,
     file = file.path(kDataPath,"rules_data.RData"))
