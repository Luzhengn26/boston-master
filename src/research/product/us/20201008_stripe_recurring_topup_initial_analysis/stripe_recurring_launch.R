...
kDataPath <- file.path("/Users/danielmermelstein/src/boston/research/product/deep_dive/stripe_recurring_topup_initial_analysis_20201008","data")
...
select 
  ...
  case when t.type = 'TopUp' and t.direction = 'Incoming' then row_number() over (partition by t.user_id order by t.completed_tstamp) else 0 end as num
...
where t.completed_tstamp::date between '",start.date,"' and '",end.date,"'
...
with txns as (
  select 
    ...
    t.completed_tstamp,
    t.bank_balance_impact_cents/100::float as txn_amount,
    case when t.type = 'TopUp' then row_number() over (partition by t.user_id order by t.completed_tstamp) else 0 end as num
  from dbt.zrh_transactions t
