---
title: "Who Are Our Users? Direct Deposit MAU 90 Days"
author: "Dani Mermelstein"
date: "2020-11-25"
region: "US"
link: "https://docs.google.com/presentation/d/1SdIY7bRBXaySL-8hpSrHvUhhFywpIE-e6VIRZT9QoJo"
tags: "direct deposit, salaried users, demographics, product strategy, spending behavior, income analysis, product, traveling, know your customers, understand users, geography, population density, account funding, atm usage, international"
summary: "This is a refresh of a previous piece of research from spring 2020, with a focus on Direct Deposit users and their behaviors within 90 days of becoming First-Time MAU"
output: 
  html_document:
    toc: TRUE
    theme: cosmo
...
# load the data
load(paste0("data/mau_90_dir_dep.RData"))

# set the datasets we're working on
demo_df <- mau_90_demo_dir_dep
funding_df <- mau_90_funding_dir_dep
spending_df <- mau_90_spending_dir_dep

```

# MAU 90 Days

Our data cover users who became MAU between `r as.Date(min(demo_df$first_txn))` and `r as.Date(max(demo_df$first_txn))` and who have made a Direct Deposit at any point during their MAU tenure. We look at those users' financial behavior in their first 90 days after becoming MAU.
...
states <- merge(states, read.csv('~/src/boston/research/product/deep_dive/who_are_our_users_20200430_US/data/state_abrev.csv', stringsAsFactors = FALSE), by.x = "NAME", by.y="State")
...
ggplot(demo_df[, .(users=.N), by=.(zip_code)][order(-users),], aes(x=users))+
  geom_histogram()+
  # xlim(-200,500)+
  labs(title="Number of Users per Zip Code: MAU First 90 Days",
       x ="Users", 
       y = "Zip Codes")
...
# print(paste0(percent(sum(density[which(Density.Per.Sq.Mile >= 5000),]$X2010.Population)/sum(density$X2010.Population)), " of this population is urban" ))
# fix data type
...
# do national calcs for indexing
national_density <- density
national_density$urbanicity <- ifelse(national_density$Density.Per.Sq.Mile >= 5000, 'urban', 
       ifelse(national_density$Density.Per.Sq.Mile >= 3000, 'inner_suburbs', 
              ifelse(national_density$Density.Per.Sq.Mile >= 890, 'outer_suburbs', 
                     ifelse(national_density$Density.Per.Sq.Mile >= 100, 'urban_fringe', 'rural'))))

national_density <- national_density[, .(people=sum(X2010.Population)), by=.(urbanicity)]
national_density$total_pop <- national_density[, .(total_pop=sum(people))]
national_density$urban_pct <- national_density$people/national_density$total_pop

# add national ratios to N26 density calcs
density_summary <- merge(density_summary, national_density[,c("urbanicity","urban_pct")], by="urbanicity", all.x=TRUE)
# create index vs national urbanicity ratios
density_summary$index <- density_summary$pct_users/density_summary$urban_pct -1

# print table
kable(density_summary[,c("urbanicity","urbanicity_rank","pct_users","index")][order(-urbanicity_rank),])%>%
  kable_styling(full_width = F)
...
For months where users made transactions, what was their average net cash flow?
Excludes microdeposits and internal txns

```{r echo=FALSE, message=FALSE}
# get monthly balance by user, then avg balance across those months per user

sapply(funding_df, typeof)
# funding_df$amount_cents <- as.integer(funding_df$amount_cents)
# sapply(funding_df, typeof)
head(funding_df)

avg_funding <- funding_df[which(is_internal_txn == FALSE),][, .(balance = sum(amount_cents)), by=.(user_id, month, mau_group)][, .(avg_balance = mean(balance)/100), by=.(user_id, mau_group)]

avg_funding

ggplot(avg_funding, aes(x=avg_balance))+
  geom_histogram()+
  xlim(-100,100)+
  labs(title="Average Monthly Net Cash Flow: MAU First 90 Days",
       x ="Average Monthly Net Cash Flow, Dollars", 
       y = "User Count")+
  facet_wrap(~mau_group)
...
ggplot(funding[!is.na(company) & mau_group == 'unbroken_mau',][order(-pct)][c(1:7,9:11)], aes(x=reorder(company, pct), y=pct))+
  geom_bar(stat = "identity")+
  scale_y_continuous(labels = percent)+
  coord_flip()+
...
ggplot(funding[!is.na(company) & mau_group == 'mau_lapsed_mau',][order(-pct)][c(1:9,11)], aes(x=reorder(company, pct), y=pct))+
  geom_bar(stat = "identity")+
  scale_y_continuous(labels = percent)+
  coord_flip()+
...
ggplot(funding[!is.na(company) & mau_group == 'mau_lapsed_lapsed_short',][order(-pct)], aes(x=reorder(company, pct), y=pct))+
  geom_bar(stat = "identity")+
  scale_y_continuous(labels = percent)+
  coord_flip()+
...
ggplot(funding[!is.na(company) & mau_group == 'mau_lapsed_lapsed_long',][order(-pct)][c(1:4,6:11)], aes(x=reorder(company, pct), y=pct))+
  geom_bar(stat = "identity")+
  scale_y_continuous(labels = percent)+
  coord_flip()+
...
ggplot(funding_df[which(type=='DIR_DEP'),][, .(amount=sum(amount_cents)/100,
  ...
  geom_histogram()+
  xlim(-200,75000)+
  labs(title="Projected Annual Income (post-tax): MAU First 90 Days",
       x ="Annual Income", 
       y = "User Count")+
  facet_wrap(~mau_group)
